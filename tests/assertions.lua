...
-- Minimal assertion helpers for the mini.test suite.
-- Adapted from https://github.com/rcasia/neotest-java/blob/main/tests/assertions.lua
--
-- `eq` replaces luassert's `assert.are.same` / `assert.are.equal` with a
-- deep-equality check that prints a full diff on failure. Everything else
-- uses plain Lua `assert` or `MiniTest.expect.*` directly.
